...
text_encoder = LlamaModel.from_pretrained("/nfs/mzh/VideoGeneration/TeaCache_old/TeaCache4HunyuanVideo/HunyuanVideo/ckpts", subfolder='text_encoder', torch_dtype=torch.float16).cpu()
text_encoder_2 = CLIPTextModel.from_pretrained("/nfs/mzh/VideoGeneration/TeaCache_old/TeaCache4HunyuanVideo/HunyuanVideo/ckpts", subfolder='text_encoder_2', torch_dtype=torch.float16).cpu()
tokenizer = LlamaTokenizerFast.from_pretrained("hunyuanvideo-community/HunyuanVideo", subfolder='tokenizer')
tokenizer_2 = CLIPTokenizer.from_pretrained("hunyuanvideo-community/HunyuanVideo", subfolder='tokenizer_2')
vae = AutoencoderKLHunyuanVideo.from_pretrained("hunyuanvideo-community/HunyuanVideo", subfolder='vae', torch_dtype=torch.float16).cpu()
...
@torch.no_grad()
def worker(input_image, prompt, n_prompt, seed, total_second_length, latent_window_size, steps, cfg, gs, rs, gpu_memory_preservation, use_magcache, mp4_crf, magcache_thresh=0.1, magcache_K=3, magcache_retention_ratio=0.2):
    ...
    try:
        ...
        for latent_padding in latent_paddings:
            ...
            if use_magcache:
                ...
                transformer.__class__.forward = magcache_framepack_forward # magcache_framepack_calibration
...
def process(input_image, prompt, n_prompt, seed, total_second_length, latent_window_size, steps, cfg, gs, rs, gpu_memory_preservation, use_magcache, mp4_crf, magcache_thresh=0.1, magcache_K=3, magcache_retention_ratio=0.06):
    global stream
    ...
    async_run(worker, input_image, prompt, n_prompt, seed, total_second_length, latent_window_size, steps, cfg, gs, rs, gpu_memory_preservation, use_magcache, mp4_crf, magcache_thresh, magcache_K, magcache_retention_ratio)
...
with block:
    ...
    with gr.Row():
        with gr.Column():
            ...
            with gr.Group():
                use_magcache = gr.Checkbox(label='Use magcache', value=False, info='Faster speed, but often makes hands and fingers slightly worse.')
                magcache_thresh = gr.Slider(label="MagCache_thresh", minimum=0.0, maximum=1.0, value=0.06, step=0.005, info='Decrease this value when the quality is poor. It denotes the accumulated error caused by skipping steps.') 
    ...
    ips = [input_image, prompt, n_prompt, seed, total_second_length, latent_window_size, steps, cfg, gs, rs, gpu_memory_preservation, use_magcache, mp4_crf, magcache_thresh, magcache_K, magcache_rention_ratio]
